...
class SlackNotifier:
    ...
    async def send_icp_upgrade_alert(
        self,
        company_id: str,
        company_name: str,
        old_tier: str,
        ...
        """
        Send Slack alert when a company's ICP tier upgrades.

        Args:
            company_id: UUID of the company
            company_name: Name of the company
            old_tier: Previous tier (e.g., BRONZE)
            new_tier: New tier (e.g., SILVER)
            old_score: Previous ICP score (optional)
            new_score: New ICP score (optional)
        ...
        if old_score is not None and new_score is not None:
            score_text = f" ({old_score} → {new_score})"
            ...
            "blocks": [
                {
                    ...
                    "text": {
                        "type": "mrkdwn",
                        "text": f"🚀 *ICP Upgrade!* {company_name}\n{old_emoji} {old_tier} → {new_emoji} {new_tier}{score_text}"
                    }
                },
                {
                    "type": "context",
                    "elements": [
                        {
                            "type": "mrkdwn",
                            "text": f"Company ID: `{company_id[:8]}...` | Detected by ICP Checker"
                        }
                    ]
                }
            ],
            "text": f"ICP Upgrade: {company_name} → {new_tier}"
        }

        return await self._send_message(payload, company_id, f"ICP Upgrade: {company_name}")
    ...
    async def send_morning_briefing(
        ...
        """
        Send morning briefing notification to Slack.

        Args:
            leads: List of lead dicts with rank, company_name, why_now, etc.
            summary: Formatted markdown summary

        Returns:
            True if notification sent successfully
        """
        # Build blocks for leads
        blocks = [
            {
                "type": "header",
                "text": {
                    "type": "plain_text",
                    "text": f"🌅 Morning Briefing - Top {len(leads)} Leads",
                    "emoji": True
                }
            },
            {
        ...
        # Add each lead
        for lead in leads[:5]:  # Only show top 5 in Slack (full list in summary)
            rank = lead.get('rank', '?')
            company = lead.get('company_name', 'Unknown')
            phone = lead.get('phone', 'No phone')
            tier = lead.get('icp_tier', 'N/A')
            why_now = lead.get('why_now', 'Strong ICP match')[:200]
        ...
        blocks.append({
            ...
            "elements": [
                {
                    "type": "mrkdwn",
                    "text": f"📊 {len(leads)} leads ranked | Generated by Prediction Market Agent"
                }
            ]
        })
        ...
        payload = {
            "blocks": blocks,
            "text": f"Morning Briefing: {len(leads)} leads ready to call"
        }

        return await self._send_message(payload, "briefing", "Morning Briefing")
