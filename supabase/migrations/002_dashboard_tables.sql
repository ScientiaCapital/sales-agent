...
-- Add close_lead_id column if table exists but column doesn't
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns
                   WHERE table_name = 'close_activities' AND column_name = 'close_lead_id') THEN
        ALTER TABLE close_activities ADD COLUMN close_lead_id VARCHAR(100);
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns
                   WHERE table_name = 'close_activities' AND column_name = 'activity_date') THEN
        ALTER TABLE close_activities ADD COLUMN activity_date TIMESTAMPTZ;
    END IF;
END $$;

-- Indexes for close_activities (only create if column exists)
